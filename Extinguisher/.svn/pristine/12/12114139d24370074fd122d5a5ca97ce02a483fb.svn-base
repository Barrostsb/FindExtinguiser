...
public class frmCadastrarManutInsp extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		lblCodigoExtintor.setText("Código do Extintor");
		...
		jComboBox1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Todos os tipos", "Água", "CO2", "Pó Químico", "Espuma Mecânica" }));
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				...
				if (manu.getId() == 0) {
					...
					JOptionPane.showMessageDialog(null, cBoxManutInsp.getSelectedItem() + " salva com sucesso!", "Confirmação", JOptionPane.INFORMATION_MESSAGE);
					clear();
					populateJTable(null, null);
				} else {
					businessManutencao.update(manu);

					JOptionPane.showMessageDialog(null, cBoxManutInsp.getSelectedItem() + " atualizada com sucesso!", "Confirmação", JOptionPane.INFORMATION_MESSAGE);
